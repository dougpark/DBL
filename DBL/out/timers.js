function addTimer(timer) {
    ...
    if (timer.second == undefined)
        timer.second = 0;
    if (timer.message == undefined)
        timer.message = preferences.defaultMessage;
    if (timer.chime == undefined)
        timer.chime = preferences.defaultChime;
    timer.sortBy = 0;
    timer.type = "speak";
    timer.kind = "timer";
...
function newTimerButton({ hour = 0, minute = 0, second = 0, chime = preferences.defaultChime, message = preferences.defaultMessage }) {
    //sounds.beep.play();
    playChime('beepon');
    addTimer({
        hour: hour,
